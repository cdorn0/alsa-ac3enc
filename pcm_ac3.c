...
static const float zeros[6 * A52_SAMPLES_PER_FRAME] = {};
...
int ac3enc_destroy(AftenContext* c, uint8_t *output) {
    int result = 0;

    if (output) {
        /* Create a dummy frame of silence */
        int outsize = ac3enc_convert(c, zeros, output);
        if (outsize >= 0) {
            ac3enc_fillIEC958Header(output, outsize);
...
struct ac3plug_info {
    ...
    float pcm_buffer[A52_SAMPLES_PER_FRAME * 6];
    int16_t ac3_buffer[IEC958_FRAME_SIZE];
                                          ...
                                          snd_pcm_uframes_t size)
{
    ...
    /* Buffer must not be overfilled, limit count of the samples copied to A52_SAMPLES_PER_FRAME */
    snd_pcm_uframes_t remaining = A52_SAMPLES_PER_FRAME - ac3plug->bufpos;
    if (size > remaining)
        size = remaining;

    int srcbufidx = ext->channels * ac3plug->bufpos;
    int dstbufidx = 2 * ac3plug->bufpos;

    for (snd_pcm_uframes_t i = 0; i < size; i++) {
        /* Read samples to pcm buffer according to channel count*/
        switch (ext->channels) {
            case 1:
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[0], i + src_offset, ext->format);
                break;
            case 2:
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[0], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[1], i + src_offset, ext->format);
                break;
            case 3:
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[0], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[1], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[2], i + src_offset, ext->format);
                break;
            case 4:
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[0], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[1], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[2], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[3], i + src_offset, ext->format);
                break;
            case 5:
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[0], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[4], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[1], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[2], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[3], i + src_offset, ext->format);
                break;
            case 6:
                ...
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[2], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[3], i + src_offset, ext->format);
                ac3plug->pcm_buffer[srcbufidx++] = get_float(&src_areas[5], i + src_offset, ext->format);
                break;
        ...
        put_s16(&dst_areas[0], i + dst_offset, ac3plug->ac3_buffer[dstbufidx++]);
        put_s16(&dst_areas[1], i + dst_offset, ac3plug->ac3_buffer[dstbufidx++]);
    }

    ac3plug->bufpos += size;

    if (ac3plug->bufpos == A52_SAMPLES_PER_FRAME) {
        int outsize = ac3enc_convert(ac3plug->afctx, ac3plug->pcm_buffer,
                                     (uint8_t *) ac3plug->ac3_buffer + IEC958_HEADER_SIZE);
        if (outsize < 0) {
        ...
        ac3enc_fillIEC958Header((uint8_t *) ac3plug->ac3_buffer, outsize);
...
static int ac3plug_init(snd_pcm_extplug_t *ext)
{
    ...
    /* Create a dummy frame of silence */
    int outsize = ac3enc_convert(ac3plug->afctx, zeros, (uint8_t *) ac3plug->ac3_buffer);
    if (outsize < 0) {
        SNDERR("AC3 conversion failed");
...
static int ac3plug_close(snd_pcm_extplug_t *ext)
{
    ...
    ac3enc_destroy(ac3plug->afctx, NULL);
    ac3plug->afctx = NULL;
    return 0;
}
...
SND_PCM_PLUGIN_DEFINE_FUNC(ac3)
{
    ...
    static const int channels[6] = {1, 2, 3, 4, 5, 6};
    static const int formats[3] = {SND_PCM_FORMAT_FLOAT, SND_PCM_FORMAT_S32, SND_PCM_FORMAT_S16};
